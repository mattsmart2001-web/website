...
<body>
    ...
    <script>
        ...
        async function fetchDRHistory() { if (drHistoryCache) return drHistoryCache; try { const apiUrl = \\\`https://gtstats.live/api/getDriverStatsHistory?user_id=\\\${CONFIG.USER_ID}&psn=\\\${encodeURIComponent(CONFIG.PSN_ID)}\\\`; const proxyUrl = \\\`https://api.allorigins.win/get?url=\\\${encodeURIComponent(apiUrl)}\\\`; console.log('Fetching DR history from:', proxyUrl); const controller = new AbortController(); const timeoutId = setTimeout(() => controller.abort(), 15000); const response = await fetch(proxyUrl, { signal: controller.signal }); clearTimeout(timeoutId); if (!response.ok) throw new Error(\\\`HTTP \\\${response.status}: \\\${response.statusText}\\\`); const proxyData = await response.json(); const data = JSON.parse(proxyData.contents); drHistoryCache = data; return data; } catch (error) { if (error.name === 'AbortError') console.error('DR history request timeout after 15 seconds'); console.error('Error fetching DR history:', error); return null; } }
        async function fetchGT7Stats() { try { const apiUrl = \\\`https://gtstats.live/api/getDriverRatingPSN?psn=\\\${encodeURIComponent(CONFIG.PSN_ID)}\\\`; const url = CONFIG.USE_CORS_PROXY ? \\\`https://api.allorigins.win/get?url=\\\${encodeURIComponent(apiUrl)}\\\` : apiUrl; console.log('Fetching from:', url); const controller = new AbortController(); const timeoutId = setTimeout(() => controller.abort(), 15000); const response = await fetch(url, { signal: controller.signal }); clearTimeout(timeoutId); if (!response.ok) throw new Error(\\\`HTTP \\\${response.status}: \\\${response.statusText}\\\`); const data = CONFIG.USE_CORS_PROXY ? JSON.parse((await response.json()).contents) : await response.json(); return data; } catch (error) { if (error.name === 'AbortError') { console.error('Request timeout after 15 seconds'); throw new Error('Request timeout'); } console.error('Error fetching GT7 stats:', error); throw error; } }
