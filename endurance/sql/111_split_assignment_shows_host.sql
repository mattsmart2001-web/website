-- ============================================================
-- 111 Split assignment message includes the split's host
--
-- The bulk "Notify Drivers" portal message told every driver their
-- split number but not who was hosting it — that only reached the
-- host themselves via the separate Hosting Instructions message.
-- Everyone else had to go find the host badge in the admin splits
-- view or wait for Discord. Add a "Host:" line, sourced from
-- whichever entry in that lobby has host_driver_id set at the time
-- Notify Drivers is clicked (auto-allocate already assigns willing
-- hosts before this step in the normal workflow, so it's usually
-- already known). Omitted entirely if no host is assigned yet.
-- ============================================================
