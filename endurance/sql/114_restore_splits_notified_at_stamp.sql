-- ============================================================
-- 114 Restore the splits_notified_at stamp in notify_lobby_assignments
--
-- Regression fix. Migration 101 introduced events.splits_notified_at
-- and stamped it at the end of notify_lobby_assignments — that flag is
-- the gate the driver portal uses to reveal a non-host driver their
-- split number (hosts see theirs immediately; everyone else only once
-- splits_notified_at is set). The function was rewritten in migrations
-- 105 → 106 → 107 → 111 to enrich the message text, and the stamping
-- UPDATE was dropped along the way. Result: "Notify Drivers" sends the
-- portal messages / emails fine, but never marks the event notified, so
-- every non-host keeps seeing "TBC" for their split.
--
-- This recreates the function exactly as in 111 and restores the single
-- missing line before RETURN. No behaviour change other than the stamp.
--
-- Note: this only fixes future notifications. To reveal splits for an
-- event already notified before this migration, stamp it once by hand:
--   UPDATE events SET splits_notified_at = now() WHERE id = '<event_id>';
-- ============================================================
...
BEGIN
    ...
    lobby_hosts AS (
        SELECT DISTINCT ON (en.lobby_number)
            en.lobby_number,
            d.display_name AS host_name
        FROM   public.entries en
    ...
    driver_lobby AS (
        ...
        LEFT   JOIN lobby_hosts lh     ON lh.lobby_number = en.lobby_number
        WHERE  en.event_id     = p_event_id
          AND  en.lobby_number IS NOT NULL
    ...
    SELECT
        ...
        END,
        format(
            E'%s%s\n\nRace: %s\nDate: %s%s%s%sRestart rule: if any driver disconnects within the first lap, a restart may be called.\n\nCheck your portal and Discord ahead of the race.',
            CASE
                WHEN dl.prev_lobby_number IS NULL THEN
            ...
            CASE WHEN event_row.circuit_name IS NOT NULL
                 ...
                 ELSE '' END,
            CASE WHEN dl.host_name IS NOT NULL
                 THEN E'\nHost: ' || dl.host_name
                 ELSE '' END,
            E'\n\n'
        ),
    ...
    FROM (
        ...
        UNION ALL SELECT 'Settings Allowed: ' || CASE event_row.settings_allowed
                              WHEN 'brake_balance_only' THEN 'Brake Balance Only'
                              WHEN 'none'               THEN 'None'
                              ELSE initcap(event_row.settings_allowed)
                          END WHERE event_row.settings_allowed IS NOT NULL
        UNION ALL SELECT 'Initial Fuel: ' || CASE event_row.initial_fuel
                              WHEN '50l' THEN '50L'
                              WHEN 'default' THEN 'Default'
                              ELSE initcap(event_row.initial_fuel)
                          END WHERE event_row.initial_fuel IS NOT NULL
    ...
    -- Mark the event as notified so the driver portal reveals split
    -- numbers to non-host drivers. (Restored — see migration 101; this
    -- line was lost in the 105–111 rewrites.)
