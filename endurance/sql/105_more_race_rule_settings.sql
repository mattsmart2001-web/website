-- ============================================================
-- 105 More race-rule settings + Car Collision Penalty relabel
--
-- Adds four more GT7 Custom Race lobby settings alongside the
-- existing ones: three enum-style settings (Grip Reduction Off
-- Track, Nitrous, Engine Swap) and one boolean (Correct Vehicle
-- Course after Wall Collision). Same nullable pattern as migration
-- 98 — unset means not specified yet, not "off"/"prohibited".
--
-- Also fixes Car Collision Penalty's host-message wording: unlike
-- Shortcut/Wall Collision Penalty, GT7's Car Collision Penalty is a
-- straight on/off toggle with no "Weak" tier, so drop the "- Weak"
-- suffix that was wrongly copied from the other two penalties when
-- migration 99 added this text.
-- ============================================================

ALTER TABLE events
    ADD COLUMN IF NOT EXISTS grip_reduction_off_track text,
    ADD COLUMN IF NOT EXISTS nitrous                   text,
    ADD COLUMN IF NOT EXISTS engine_swap                text,
    ADD COLUMN IF NOT EXISTS correct_vehicle_course     boolean;
