-- ============================================================
-- 66 Notification "prior round" lookup uses round number, not date
--
-- Migration 65 used starts_at to find the previous round, which is
-- fragile: if any event in the season has a NULL starts_at (e.g.
-- a placeholder for a future round whose date is TBC), the lookup
-- silently returns nothing and every driver gets the first-time
-- "Split Assignment" copy instead of the promotion / relegation
-- variant.
--
-- The round number is NOT NULL by schema, so use that as the
-- primary ordering. starts_at is still used for the user-facing
-- "Date: …" line in the message body — only the prior-event
-- lookup needs hardening.
-- ============================================================
