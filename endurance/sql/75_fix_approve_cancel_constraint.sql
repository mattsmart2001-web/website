-- Fix duplicate key constraint in approve_team_join_request.
--
-- The unique index on (team_id, driver_id, status) means we cannot UPDATE
-- a pending request to 'cancelled' if a cancelled row already exists for
-- the same (team_id, driver_id) pair (e.g. the driver applied, cancelled,
-- then reapplied). The auto-decline step at the end of approval hit this.
--
-- Fix: DELETE the other pending requests instead of updating their status.
-- Driver-withdrawn / system-auto-declined rows don't need a status trail.
